...
class SqlHelper(object):
    def __init__(self,host="localhost",port=3306,user="",pwd="",db="test",dbtype='mysql'):
        self.host = host
        self.port = port
        self.user = user
        self.pwd = pwd
    ...
    def __GetConnect(self):
        ...
        if self.dbtype=='mysql':
            import pymysql
            self.conn=pymysql.connect(host=self.host,port=self.port,user=self.user,password=self.pwd,db=self.db,charset='utf8mb4',local_infile=True)
